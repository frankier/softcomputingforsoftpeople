...
use softcomputingforsoftpeople::{parse_seed, get_rng, Individual, Fitness, Stats};
use num_traits::sign::Signed;
use std::fs::File;

struct Hypercube<N: Scalar + Ring + Signed + SampleRange + PartialOrd, R: DimName, C: DimName>
...
impl<N: Scalar + Ring + Signed + SampleRange + PartialOrd, R: DimName, C: DimName> Hypercube<N, R, C>
        where DefaultAllocator: Allocator<N, R, C>
{
    fn new(min: MatrixMN<N, R, C>, max: MatrixMN<N, R, C>) -> Hypercube<N, R, C> {
        assert!(min < max);
        let range = &max - &min;
        Hypercube { min, max, range: range }
    }

    fn sample<G: Rng>(&self, rng: &mut G) -> MatrixMN<N, R, C>
        where DefaultAllocator: Allocator<N, R, C>
    {
    ...
    fn go_nearest_torus(&self, from: &MatrixMN<N, R, C>, to: &MatrixMN<N, R, C>) -> MatrixMN<N, R, C> {
    ...
    fn place_torus(&self, point: &MatrixMN<N, R, C>) -> MatrixMN<N, R, C> {
