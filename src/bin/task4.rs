          ...
          <DefaultAllocator as Allocator<f32, D>>::Buffer: Copy
{
    ...
    for m in 0..points[0].fitness.len() {
        ...
        indices.sort_by_key(|&idx| NotNaN::new(points[idx].fitness[m]).unwrap());
        ...
        for i in 1..(l - 1) {
            ...
            points[indices[i]].domination.crowding +=
                NotNaN::new((points[indices[i + 1]].fitness[m] -
                             points[indices[i - 1]].fitness[m]) /
                            (f_max - f_min))
                        .unwrap();
        }
    }
            ...
            .collect();
    {
        ...
        for front in fronts.iter() {
            ...
            for &elem_i in front.iter() {
                front_refs.push(unsafe {
                                    &mut *(&mut individuals[elem_i] as *mut FitnessesRankStats<U2>)
                                });
            }
